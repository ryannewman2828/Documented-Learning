...
def answer_three():
    f = df.loc[df['Gold'] > 0][df['Gold.1'] > 0]
    return ((f['Gold'] - f['Gold.1']) / (f['Gold'] + f['Gold.1'])).idxmax()
...
def answer_five():
    return census_df[["COUNTY", "STNAME"]].groupby("STNAME").sum().idxmax().values[0]
...
def answer_six():
    ...
    return county_df.sort_values(by=['STNAME','CENSUS2010POP'],ascending=False).groupby('STNAME').head(3).groupby('STNAME').sum().sort_values(by='CENSUS2010POP').head(3).index.tolist()
...
def answer_seven():
    ...
    return counties_df['CTYNAME'][counties_df['pop_change'] == max(counties_df['pop_change'])].tolist()[0]
...
def answer_eight():
    counties_df = census_df[census_df['SUMLEV'] == 50]
    return counties_df[((counties_df['REGION'] == 1) |
