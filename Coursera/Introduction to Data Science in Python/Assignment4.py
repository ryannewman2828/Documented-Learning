...
def get_list_of_university_towns():
    ...
    frames = []
    for town in towns:
        if re.match('.*\[edit\]', town) is not None:
            curr_town = town[:town.index('[edit]')]
        elif re.match('.*[(].*', town) is not None:
            frames.append([curr_town, town[:town.index('(') - 1]])
        else:
            frames.append([curr_town, town])    
    return pd.DataFrame(frames, columns=["State", "RegionName"])
...
def run_ttest():
    ...
    data['up&down'] = data.apply(lambda x: (x['2008q3'] - x['2009q2']) / x['2008q3'], axis=1)
    
    uni_town = get_list_of_university_towns()['RegionName']
    uni_town = set(uni_town)

    data['is_uni'] = data.apply(lambda x: 1 if x['RegionName'] in uni_town else 0, axis=1)    
    
    not_uni = data[data['is_uni'] == 0].loc[:, 'up&down'].dropna()
    is_uni  = data[data['is_uni'] == 1].loc[:, 'up&down'].dropna()
    
    final = 'non-university town' if not_uni.mean() < is_uni.mean() else 'university town'
       
    p_val = list(ttest_ind(not_uni, is_uni))[1]
    result = (True, p_val, final)
    return result
